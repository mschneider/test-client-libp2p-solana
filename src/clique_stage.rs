use std::{
  ...
  hash::{Hash, Hasher},
  str::FromStr,
  time::Duration, thread::{JoinHandle, Builder}, sync::{Arc, atomic::{AtomicBool, Ordering}}, task::{Poll, Context}, 
};
...
use solana_perf::packet::{PacketBatch, Packet, Meta, PACKET_DATA_SIZE};
...
impl CliqueStage {
  ...
  pub fn new(
      ...
      let clique_thread_hdl = 
          ...
          let secret_key = identity::ed25519::SecretKey::from_bytes(copy).expect("CliqueStage solana_keypair is ed25519 compatible");
          ...
          let transport = tcp::async_io::Transport::default()
              ...
              .authenticate(noise::NoiseAuthenticated::xx(&local_key.clone()).expect("CliqueStage noise authentication available"))
          ...
          */
          ...
          gossipsub.subscribe(&topic).expect("CliqueStage subscribe to topic");
          ...
          for to_dial in std::env::args().skip(1) {
              let addr = Multiaddr::from_str(&to_dial).expect("CliqueStage valid peer Multiaddr");
              swarm.dial(addr).expect("dial succeeds");
          }

          // Listen on all interfaces
          swarm.listen_on("/ip4/0.0.0.0/tcp/8765".parse().expect("CliqueStage valid listen Multiaddr")).expect("CliqueStage listen succeeds");
          ...
          loop {
              if config.exit.load(Ordering::Relaxed) { break; }
                  ...
                  if let Ok(outbound) = clique_outbound_receiver.try_recv() {
                      for shred in outbound.iter() {
                          if let Err(e) = swarm
                              ...
                              trace!("CliqueStage outbound publish error: {}", e);
                              break;
                          }
                      }
                  ...
                  if let Poll::Ready(Some(inbound)) = swarm.poll_next(&mut cx) {
                      match inbound {
                          SwarmEvent::NewListenAddr { address, .. } => {
                              trace!("CliqueStage listening on {address:?}");
                          }
                          SwarmEvent::Behaviour(SolanaCliqueBehaviourEvent::Identify(event)) => {
                              trace!("CliqueStage identify: {event:?}");
                          }
                          SwarmEvent::Behaviour(SolanaCliqueBehaviourEvent::Gossipsub(gossipsub::GossipsubEvent::Message {
                              propagation_source: _peer_id,
                              message_id: _id,
                              message,
                          })) => {
                              if let Ok(packet_bytes) = TryInto::<[u8;PACKET_DATA_SIZE]>::try_into(message.data.as_slice()) {
                                  let uni_batch = PacketBatch::new(vec![Packet::new(packet_bytes, Meta::default())]);
                                  clique_inbound_sender.send(uni_batch).expect("CliqueStage send inbound");
                              }
                          }
                          SwarmEvent::Behaviour(SolanaCliqueBehaviourEvent::Ping(event)) => {
                              match event {
                                  ...
                                  ping::Event {
                                      ...
                                      trace!("CliqueStage ping: pong from {}", peer.to_base58());
                                  }
                                  ping::Event {
                                      peer,
                                      result: Result::Err(ping::Failure::Timeout),
                                  } => {
                                      trace!("CliqueStage ping: timeout to {}", peer.to_base58());
                                  }
                                  ping::Event {
                                      peer,
                                      result: Result::Err(ping::Failure::Unsupported),
                                  } => {
                                      trace!("CliqueStage ping: {} does not support ping protocol", peer.to_base58());
                                  }
                                  ping::Event {
                                      peer,
                                      result: Result::Err(ping::Failure::Other { error }),
                                  } => {
                                      trace!("CliqueStage ping: ping::Failure with {}: {error}", peer.to_base58());
                                  }
                              }
                          }
                          _ => {}
